...
from storyteller.algorithm.mcts_action import SelectNextVisualizationTaskAction, GenerateVisualizationAction, ReviseVisualizationAction, GenerateCaptionAction, GenerateChapterSummaryAction
import copy
...
generate_chapter_summary_action = GenerateChapterSummaryAction()
...
# 测试流程
print("开始测试可视化图表生成、修改、说明生成和章节总结流程...")
...
if next_task_nodes:
    ...
    print(f"  选择了任务: {selected_task['task_name']}")
    print(f"  章节: {selected_task['chapter_title']}")
                ...
                if revised_chapter.charts:
                    ...
                    if caption_nodes:
                        ...
                        if caption_chapter.charts:
                            ...
                            # 设置选定的任务
                            caption_node.selected_task = selected_task
                            
                            # 执行章节总结生成操作
                            summary_nodes = generate_chapter_summary_action.create_children_nodes(caption_node, llm_kwargs)
                            
                            if summary_nodes:
                                summary_node = summary_nodes[0]
                                summary_chapter = summary_node.report.chapters[chapter_idx]
                                
                                # 检查是否生成了章节总结
                                if hasattr(summary_chapter, 'summary') and summary_chapter.summary:
                                    print(f"  成功生成章节总结:")
                                    print(f"    {summary_chapter.summary}")
                                else:
                                    print("  没有生成章节总结")
                            else:
                                print("  生成章节总结失败")
